        ...
        <div class="max-w-6xl mx-auto bg-white shadow rounded-lg p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3">
                <form action="{{ route('admin.matkul.index') }}" method="GET" class="flex gap-2">
                    <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari kode, nama, prodi..."
                    ...
                    <tbody>
                        @forelse ($matkul as $index => $m)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border px-4 py-2">{{ $m->sks }}</td>
                                <td class="border px-4 py-2">{{ $m->semester }}</td>
                                <td class="border px-4 py-2">{{ $m->prodi->name ?? '-' }}</td>
                                <td class="border px-4 py-2">{{ $m->dosen->nama_dosen ?? '-' }}</td>
            ...
            <div class="md:hidden space-y-4">
                @forelse ($matkul as $m)
                    <div class="border rounded-lg p-4 shadow-sm">
                        ...
                        <p class="text-gray-600 text-sm">Kode MK: {{ $m->kode_mk }}</p>
                        <p class="text-gray-600 text-sm">SKS: {{ $m->sks }} | Semester: {{ $m->semester }}</p>
                        <p class="text-gray-600 text-sm mb-2">Prodi: {{ $m->prodi->name ?? '-' }}</p>
                        <p class="text-gray-600 text-sm mb-2">Dosen: {{ $m->dosen->nama_dosen ?? '-' }}</p>
            ...
            {{-- Pagination --}}
            <div class="mt-4">
                {{ $matkul->appends(['search' => $search])->links() }}
            </div>
        </div>
    </div>
