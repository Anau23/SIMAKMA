<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Jadwal Mata Kuliah') }}</h2>
    </x-slot>

    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-3xl mx-auto bg-white shadow rounded-lg p-6">

            <form action="{{ route('admin.jadwal_matkul.update', $jadwal_matkul->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    ...
                    <select name="kelas_id" class="w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        @foreach ($kelas as $k)
                            <option value="{{ $k->id }}"
                                {{ old('kelas_id', $jadwal_matkul->kelas_id) == $k->id ? 'selected' : '' }}>
                                {{ $k->name }}</option>
                        @endforeach
                    </select>
                    @error('kelas_id')
                ...
                <div class="mb-4">
                    ...
                    <select name="matkul_id" class="w-full border-gray-300 rounded-md shadow-sm">
                        ...
                        @foreach ($matkuls as $m)
                            <option value="{{ $m->id }}"
                                {{ old('matkul_id', $jadwal_matkul->matkul_id) == $m->id ? 'selected' : '' }}>
                                {{ $m->name }}</option>
                        @endforeach
                    </select>
                    @error('matkul_id')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="hari" value="{{ old('hari', $jadwal_matkul->hari) }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="time" name="jam_mulai"
                        value="{{ old('jam_mulai', $jadwal_matkul->jam_mulai->format('H:i')) }}"
                        class="w-full border-gray-300 rounded-md shadow-sm">
                    @error('jam_mulai')
                        <p class="text-red-600 text-sm">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="time" name="jam_selesai"
                        value="{{ old('jam_selesai', $jadwal_matkul->jam_selesai->format('H:i')) }}"
                        class="w-full border-gray-300 rounded-md shadow-sm">
                    @error('jam_selesai')
                        <p class="text-red-600 text-sm">{{ $message }}</p>
                    @enderror
